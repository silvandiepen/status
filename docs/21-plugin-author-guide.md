...
Never put client secrets in the plugin package. OAuth `redirectUri` must use `com.statusfoundry.status.oauth:/{provider-slug}` and the slug must exactly match `auth.provider`. Also list the authorization and token endpoint hosts in `manifest.json` `domains`; Status makes those network calls during connect and refresh.
