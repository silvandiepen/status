...
- The app shells can run an installed declarative plugin request through `PluginRuntimeService`: load the installed package, upsert the account, create a manual job, execute the request/mapping pipeline, persist resources/events/status items, and write a job audit entry. Website Uptime also has the first native setup path for a user-configured host, persisted as non-secret account configuration.
