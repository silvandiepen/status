...
- `PluginPackageVerifier` now enforces package SHA-256 matches, Ed25519 signatures against the app-pinned development signing key, and registry revocation checks before a package can be treated as installable. Production distribution must replace the repository development key with release key custody described in `docs/07-security-privacy.md`.
