...
Provider application IDs for OAuth belong in `auth.json` as `provider` and `applicationId` (public client IDs only). OAuth `redirectUri` must use `com.statusfoundry.status.oauth:/{provider-slug}` with a slug matching `auth.provider`. Declare the OAuth authorization and token endpoint hosts in `manifest.json` `domains`.
